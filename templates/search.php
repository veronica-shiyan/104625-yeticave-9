        ...
        <?php if ($pages_count > 1): ?>
            <ul class="pagination-list">
                <li class="pagination-item pagination-item-prev">
                    <a href="<?php if ($current_page > 1) : ?>search.php?search=<?= esc($search) ?>&page=<?= (esc($current_page) - 1); ?><?php endif; ?>">Назад</a>
                </li>
                <?php foreach ($pages as $page): ?>
                    <li class="pagination-item <?php if ($page == $current_page): ?>pagination-item-active<?php endif; ?>">
                        <a href="search.php?search=<?= esc($search) ?>&page=<?= $page; ?>"><?= $page; ?></a>
                    </li>
                <?php endforeach; ?>
                <li class="pagination-item pagination-item-next">
                    <a href="<?php if ($current_page < count($pages)) : ?>search.php?search=<?= esc($search) ?>&page=<?= ((int)esc($current_page) + 1); ?><?php endif; ?>">Вперед</a>
                </li>
            </ul>
        <?php endif; ?>
    <?php else: ?>
